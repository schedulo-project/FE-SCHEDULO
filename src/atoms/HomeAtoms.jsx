...
export const handelCheckAtom = atom(null, async (get, set, id) => {
  ...
  const newList = todoList.map((t) => (t.id === id ? updatedTask : t));
...
export const handleChangeAtom = atom(null, (get, set, { data, id }) => {
...
const ScheduleCompleteOrNot = async ({ data }) => {
  ...
  const tags = tagNames ? tagNames.split(",").map((tag) => tag.trim()) : [];
  const id = `${data.id}/`;
